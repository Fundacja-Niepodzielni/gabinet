...
7. **`horizon` i `docs/api` są NIEZMIERZONE, nie czyste** — wyłączone ze skanu zasięgu
   unieważnienia. Waga zależy od tego, co tam wystawiamy.
8. **Brak kontroli, czy `dc()` nadal eksportuje cztery zmienne** — dług nazwany 09.08.
9. **`ZadanieRetencji` — klucz nie-całkowity** — refaktor odłożony świadomie.
10. **Klasa 3 otwarta: `R6B-8`, `R6B-6`.**

### Niski iloczyn / zablokowane

11. **`R6B-1` i `N-12` zależą od nogi 1** — nie ruszaj przed naprawą przyrządu.
12. **Przeprojektowanie `TwierdzeniaKomentarzyTest`** (kontrola D3, zdjęta z bramki: 14 obejść
...
5. **Pozycję zamyka PLIK, nie meldunek w oknie** (`S-3`): `ODPOWIEDZ` → commit/push → `PODJETO`
   → dopiero meldunek.
6. **NIGDY nie otwieraj pliku do zapisu przed przygotowaniem treści.** `open(p,'w')` obcina
   plik NATYCHMIAST; gdy zapis padnie, zostaje ZERO bajtów — a `git add -A` zacommituje tę
   pustkę bez mrugnięcia. **09.08 skasowałem tak ten plik i WYPCHNĄŁEM skasowanie**, bo
   `git add -A` w tym samym poleceniu nie pytał, co się zmieniło. Zapisuj do `.nowy`,
   sprawdź rozmiar i znaczniki, dopiero podmieniaj — i **nie łącz zapisu z `git add -A`
   w jednym poleceniu**.
7. **Brak informacji ≠ informacja o braku.** Każde studium kończ sekcją „czego nie znalazłem".
